...
class StartDiagnosisUseCaseTest {
    ...
    private val useCase = StartDiagnosisUseCase(
        orders = orders,
        events = events,
    )
    ...
    @Test
    fun `throws when the order is not received`() {
    ...
    @Test
    fun `starts diagnosis on a received order`() {
        ...
        assertEquals(
            expected = ServiceOrderStatus.IN_DIAGNOSIS.name,
