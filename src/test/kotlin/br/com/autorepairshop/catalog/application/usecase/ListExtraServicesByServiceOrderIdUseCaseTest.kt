...
@Tag("unit")
class ListExtraServicesByServiceOrderIdUseCaseTest {
    ...
    private val useCase = ListExtraServicesByServiceOrderIdUseCase(
        extras = extras,
        orders = orders,
        access = access,
    )
    ...
    @Test
    fun `maps persisted extras of the order to responses`() {
        val order = ServiceOrderFixtures.budgetApproved()
        val serviceOrderId = order.id.value
        val first = CatalogFixtures.extraService(name = "Pendente", serviceOrderId = serviceOrderId)
        val second = CatalogFixtures.extraService(name = "Aprovado", serviceOrderId = serviceOrderId)
        every { orders.findById(id = ServiceOrderId(value = serviceOrderId)) } returns order
        every { extras.findByServiceOrderId(serviceOrderId = serviceOrderId) } returns
    ...
    @Test
    fun `returns empty list when the order has no extras`() {
        val order = ServiceOrderFixtures.budgetApproved()
        val serviceOrderId = order.id.value
        every { orders.findById(id = ServiceOrderId(value = serviceOrderId)) } returns order
        every { extras.findByServiceOrderId(serviceOrderId = serviceOrderId) } returns emptyList()
