...
class DeliverServiceOrderUseCaseTest {
    ...
    private val useCase = DeliverServiceOrderUseCase(
        orders = orders,
        events = events,
    ...
    @Test
    fun `throws when the order is not completed`() {
    ...
    @Test
    fun `delivers a completed order`() {
        val order = ServiceOrderFixtures.completed()
        every { orders.findById(id = order.id) } returns order
        ...
        assertEquals(
            expected = ServiceOrderStatus.DELIVERED.name,
